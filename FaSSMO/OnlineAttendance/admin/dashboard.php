...
<body class="w3-responsive w3-content">
    ...
    <nav class="w3-panel w3-text-black w3-tiny">
		...
		<h2 class="w3-text-black w3-round-xlarge" style="background-image: url('../img/MMSU_bg.jpg'); background-repeat:no-repeat; background-size: cover;"><i class="material-icons w3-xxlarge">person</i><i class="material-icons w3-xxlarge">dashboard</i><br><b>Administrator's Dashboard</b></h2>
        <?php
        if (isset($_POST['regStudent'])) {
        ...
        else if (isset($_POST['regTeacher'])) {
            ...
            $fname = ucwords($_POST['fname']);
            $lname = ucwords($_POST['lname']);
            if(!empty($teacherID) || !empty($user) || !empty($pass) || !empty($title) || !empty($fname) || !empty($lname)){
                include "../connect2db.php";
                if (mysqli_connect_error()) {die('Connect Error('.mysqli_connect_error().')'.mysqli_connect_error());}
                else {
                    $SELECT = "SELECT teacher_ID FROM teacherscmpt WHERE teacher_ID = ? LIMIT 1";
                    $INSERT = "INSERT INTO teacherscmpt (teacher_ID,user,pass,honorific,firstname,lastname) VALUES(?,?,?,?,?,?)";
                    $krey = $conn->prepare($SELECT);
                    $krey->bind_param("s",$teacherID);
                    $krey->execute();
                    $krey->bind_result($teacherID);
                    $krey->store_result();
                    $mund = $krey->num_rows;
                    if($mund == 0){
                        $krey->close();
                        $krey = $conn->prepare($INSERT);
                        $krey->bind_param("ssssss",$teacherID,$user,$pass,$title,$fname,$lname);
                        $krey->execute();
                        echo "<h1 class='w3-green w3-animate-zoom'>Successfully Registered!</h1>";}
                    else {echo "<h1 class='w3-gray w3-animate-zoom'>This Teacher ID Number is Already Registered!</h1>";}
                    $krey->close();
                    $conn->close();}}
            else {echo "Incomplete DATA!"; die();}
        }
        ...
        else if(isset($_POST['delTeach'])) {
            include "../connect2db.php";
            if (mysqli_connect_error()) {die('Connect Error('.mysqli_connect_error().')'.mysqli_connect_error());}
            else {
            $id = $_POST['id'];
            $del = "DELETE FROM teacherscmpt WHERE teacher_id = '$id'";
            mysqli_query($conn,$del);
            echo "<h1 class='w3-green w3-center w3-animate-zoom'>Successfully Deleted!</h1>";}}
        ?>
        <div class="w3-panel w3-center w3-padding w3-round-large">
            ...
            <i class="material-icons w3-jumbo">person_add</i><br><h6><b>Register a Teacher</b></h6></button><br>
            ...
            <i class="material-icons w3-jumbo">description</i><br><h6><b>Teachers' Information</b></h6></button><br>
            ...
            <i class="material-icons w3-jumbo">date_range</i><br><h6><b>Student Attendance Record</b></h6></button>
        </div>
        ...
        <div id="Treg" class="w3-modal">
                <div class="w3-modal-content w3-card-4 w3-animate-right w3-purple w3-round" style="max-width:600px">
                    ...
                    <h2 class="w3-yellow"><b>Teacher Registration Form</b></h2>
                    ...
                    <div class="w3-margin">
                        <p><span class="w3-text-red w3-left">* required field</span></p><br>
                        <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>">
                            <label class="w3-label w3-left"><span class="w3-text-red">* </span>Teacher ID Number</label>
                            <input required class="w3-input w3-round w3-margin-bottom" type="text" name="teacherID" placeholder="Teacher ID Number" autocomplete="off">
                            <label class="w3-label w3-left"><span class="w3-text-red">* </span>Username</label>
                            <input required class="w3-input w3-round w3-margin-bottom" type="text" name="user" placeholder="Username" autocomplete="off">
                            <label class="w3-label w3-left"><span class="w3-text-red">* </span>Password</label>
                            <input required class="w3-input w3-round w3-margin-bottom" type="password" name="pass" placeholder="Password" autocomplete="off">
                            ...
                            <input required class="w3-input w3-round w3-margin-bottom" type="text" name="lname" placeholder="Last Name" autocomplete="off">
                            <br>
                            <input class="w3-button w3-hover-green w3-hover-shadow w3-yellow w3-round w3-margin" type="submit" name="regTeacher" value="Submit">  
                        </form>
                    </div>
                </div>
        </div>
                    ...
                    <div class="w3-responsive">
                        <table class="w3-table w3-green w3-margin-bottom">
                            ...
                            foreach($output as $row){
                                ...
                                <input type='submit' class='w3-button w3-red' name='delTeach' value='Remove'>
                                </form></td>
                                <td><form action='edit.php' method='post'>
                                <input hidden type='text' name='id' value='".$row['teacher_id']."'>
                                <input type='submit' class='w3-button w3-yellow' name='editTeach' value='Edit'>
                                </form></td>
        ...
        <div id="rec" class="w3-modal">
			    ...
			    $today = date('Y-m-d');
                            $sql = "SELECT * FROM timeoutcmpt INNER JOIN students on timeoutcmpt.studID = students.studID INNER JOIN timeincmpt on timeoutcmpt.studID = timeincmpt.studID WHERE timein.cmpt.datein LIKE '%today%' ORDER BY timeincmpt.num ASC";
                            $output = mysqli_query($conn,$sql);
                            foreach($output as $row){
                                echo "<tr class='w3-hover-blue w3-hover-shadow'>
                                <td>".$row['studID']."</td>
        ...
        <div id="print" class="w3-modal">
			...
			<input hidden type="text" name="today" value='<?php date_default_timezone_set("Asia/Manila"); echo date('Y-m-d');?>'>
                        ...
                        <input type="submit" class="w3-button w3-margin w3-green w3-round w3-right" name="printnow" value="Export to PDF" onclick="document.getElementById('print').style.display='none'">
                    </form>
                </div>
            </div>
        </div>
...
if(val=='student')
   ...
   {document.getElementById('Tprint').style.display='block';
    ...
    document.getElementById('acadb').style.display='block';}}</script>
</html>
